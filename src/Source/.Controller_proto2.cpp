...
void* httpRequestHandler(void* param){
	...
	do{
		...
		write(1,clientinfo->clientrequest,clientrequestlen); //dumping on the screen
	...
	if(	(tempptr = strstr(tempptr,"mimecheck") )!=NULL){
		...
		if(!strlen(clientinfo->hostname) || (!strcmp(clientinfo->hostname,controller->ipaddress) && clientinfo->hostport == controller->portnumber ) ){
			strcpy(clientinfo->hostname,"dcmtol");		
		}
...
/*				else if(strcmp(halfstr,"&")==0){
		...
		sprintf(tempstrbig,"sed 's/IPADDRESS/%s:%d/g' %s/actualjs2.js | sed 's/FILENAME/%s/g' > %s/js2.js",controller->ipaddress,controller->portnumber,HOME_DIR_PATH,"pagesave.html",HOME_DIR_PATH);
			
		system(tempstrbig);

		sprintf(jsfilename,"%s/js2.js",HOME_DIR_PATH);
		FILE *fpjs2 = fopen(jsfilename,"r");

		cout<<"File open = "<<strerror(errno)<<"\n";
	
		struct stat filestatbuff;	
		stat(jsfilename,&filestatbuff);
	 ...
	 sprintf(clientinfo->clientrequest,"HTTP/1.1 200 OK\r\nDate: Tue, 01 Mar 2011 06:14:58 GMT\r\nAccept-Ranges: bytes\r\nContent-Type: text/javascript\r\nContent-Length: %d\r\nServer: DCMTOL\r\n\r\n",contentlength);
		...
		cout<<"\n\nreply being sent to the client \n\n"<<clientinfo->clientrequest<<"\n";
		int cnt = strlen(clientinfo->clientrequest);
						
		//sending the changed file to the client
		fread(clientinfo->clientrequest + cnt,contentlength,1,fpjs2);									
		cnt = write(clientinfo->clientsockfd,clientinfo->clientrequest,cnt + contentlength);
		cout<<"sent the reply to the client's 1st Javascript, written "<<cnt<<" bytes : "<<strerror(errno)<<"\n";
		...
		if(strcmp(clientinfo->hostname,"dcmtol")==0){ 
			...
			struct stat filestatbuff;	
			stat(filepath,&filestatbuff);
			int contentlength = filestatbuff.st_size;

	    sprintf(tempstrbig,"HTTP/1.1 200 OK\r\nDate: Tue, 01 Mar 2011 06:14:58 GMT\r\nAccept-Ranges: bytes\r\nContent-Type: text/html\r\nContent-Length: %d\r\nServer: DCMTOL\r\n\r\n",contentlength);
		
			write(clientinfo->clientsockfd,tempstrbig,strlen(tempstrbig));
			filefd = open(filepath,O_RDONLY);
	
			if(filefd == -1)
				pagenotavailable = true;
			else{	
				while((cnt = read(filefd,clientinfo->clientrequest,BUFSIZ))>0)
					write(clientinfo->clientsockfd,clientinfo->clientrequest,cnt);	
			}
		}
			...
			if(hostentry==NULL)
				...
				if(clientinfo->hostsockfd == -1)
					...
					if(connect(clientinfo->hostsockfd,(struct sockaddr*)&sockhost,sizeof(struct sockaddr)) < 0)
						...
						cout<<"\n\nSending the following request to host:\n*********\n"<<clientinfo->clientrequest<<"\n**********\n\n";
						...
						if(write(clientinfo->hostsockfd,clientinfo->clientrequest,strlen(clientinfo->clientrequest)) == -1)
							...
							//READ THE STREAM AND REMOVE THE HTTP HEADER FROM IT
							cnt = read(clientinfo->hostsockfd,clientinfo->clientrequest,BUFSIZ);
							replyptr = clientinfo->clientrequest;
		
							clientinfo->clientrequest[cnt] = '\0';
	
							//step 1: to find the content length from the reply
							contenttypeptr = strcasestr(replyptr,"content-length: ");
							contenttypeptr += strlen("content-length: ");		
				
							while(*contenttypeptr != '\r')
							...
							//step 2: to find the content type
							contenttypeptr = strstr(replyptr,"Content-Type");
							...
							if(strstr(contenttype,"text/")!=NULL || !strcmp(contenttype,"application/octet-stream") || strstr(contenttype,"image/")!=NULL){
								...
								if(!strcmp(contenttype,"text/html")){
									...
									semop(controller->dbsemid,&controller->waitop,1);
										...
										sprintf(query,"select timeupdate from TIMEOUT where clientip = '%s'",clientinfo->clientipaddress);
										mysql_query(dbconnection,query);
										...
										if(row != NULL){		
											if((curtime - atol(row[0])) >  1800){
												...
												sprintf(query,"delete from TIMEOUT where clientip = '%s'",clientinfo->clientipaddress);	
												mysql_query(dbconnection,query);

												sprintf(query,"insert into TIMEOUT values ('%s','%ld')",clientinfo->clientipaddress,curtime);
												mysql_query(dbconnection,query);
											}				
											else
												pagesave = false;
									...
									if(pagesave){
										sprintf(tempfilename,"%s/pagesave.html",HOME_DIR_PATH);
										fpsendhtml = fopen(tempfilename,"w");														
	
										replyptr = strstr(clientinfo->clientrequest,"\r\n\r\n") + strlen("\r\n\r\n");
										cnt = cnt - (int) (replyptr - clientinfo->clientrequest);							
							...
							if(pagesave == false){
									replyptr = strstr(clientinfo->clientrequest,"\r\n\r\n") + strlen("\r\n\r\n");
									totalcnt = cnt - (int) (replyptr - clientinfo->clientrequest);							
							}
							...
							if(mimefound){			
								...
								while(1){
									...
									if(pagesave==true)
										fputs(replyptr,fpsendhtml);
									else{
										cout<<"had read "<<cnt<<" bytes : about to write them on the socket\n";
										cnt = write(clientinfo->clientsockfd,clientinfo->clientrequest,cnt);
										cout<<"Successfully written "<<cnt<<" bytes to the client socket totalcnt = "<<totalcnt<<"\n";
									}							
								...
								if(pagesave == true){
									...
									sprintf(tempstrbig,"sed 's/IPADDRESS/%s:%d/g' %s/actualjs1.html > %s/js1.html",controller->ipaddress,controller->portnumber,HOME_DIR_PATH,HOME_DIR_PATH);	
									system(tempstrbig);
	
									sprintf(tempfilename,"%s/js1.html",HOME_DIR_PATH);
									fpsendhtml = fopen(tempfilename,"r");															
		
									struct stat filestatbuff;	
									stat(tempfilename,&filestatbuff);
									contentlength = filestatbuff.st_size;

									sprintf(clientinfo->clientrequest,"HTTP/1.1 200 OK\r\nDate: Tue, 01 Mar 2011 06:14:58 GMT\r\nAccept-Ranges: bytes\r\nContent-Type: text/html\r\nContent-Length: %d\r\nServer: DCMTOL\r\n\r\n",contentlength);
										
									cnt = strlen(clientinfo->clientrequest);
						
									//sending the changed file to the client
									fread(clientinfo->clientrequest + cnt,contentlength,1,fpsendhtml);							
																		
									write(clientinfo->clientsockfd,clientinfo->clientrequest,cnt + contentlength);
									fclose(fpsendhtml);
								...
								if(!mimefound){
									...
									if((controller->transcodetable + transcodeindex)->transcodingstarted == true){
										...
										cout<<strerror(errno)<<" : mofo the value of streamfilefd = "<<streamfilefd<<endl;
